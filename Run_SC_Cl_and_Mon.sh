...
if [ ${LANG:0:2} = de ]
then
	ConfigfileChangedTitle=$ConfigfileChangedTitle
	ConfigfileChangedBody=$ConfigfileChangedBody
fi
...
do
	...
	LastModifiedConfigNow=`stat --format==%Y $scratchClient_configfile`
	if [ $LastModifiedConfigAtStart != $LastModifiedConfigNow ] ;
	then
		notify-send -u critical -t 2500 "$ConfigfileChangedTitle" "$ConfigfileChangedBody"	
	fi
